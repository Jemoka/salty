...
class MovementManager:
    ...
    def __init__(
        ...
        # Speech movement state
        self._assistant_speaking = False
        self._last_sway_time = 0.0
        self._sway_interval = 2.0  # Queue new sway every 2 seconds

        # DoA monitoring state
        self._doa_monitor_active = False
        self._doa_thread: threading.Thread | None = None
        self._doa_stop_event = threading.Event()
    ...
    def update_speaking_movements(self) -> None:
        """Queue sway movements while assistant is speaking.

        Call this periodically (e.g., every 100ms) from the audio loop.
        """
    ...
    def _doa_loop(self) -> None:
        ...
        while not self._doa_stop_event.is_set():
            try:
                # Get Direction of Arrival
                # 0 radians is left, π/2 radians is front/back, π radians is right
                doa, is_speech_detected = self.current_robot.media.get_DoA()
                ...
                if is_speech_detected:
                    ...
                    if current_time - self._last_turn_time >= self._turn_cooldown:
                        ...
                        if self._last_doa is None or abs(doa - self._last_doa) > 0.2:  # ~11 degrees
                            ...
                            # Get current body yaw from status
    ...
    def stop(self) -> None:
        ...
        # Stop DoA monitoring first
        self.stop_doa_monitoring()
